...
int read_problem_dense(const mxArray *label_vec, const mxArray *instance_mat, const mxArray *upbound_vec)
{
	int i, j, k;
	int elements, max_index, sc, label_vector_row_num, upbound_vector_row_num;
	double *samples, *labels, *upbound;
  ...
  upbound_vector_row_num = (int)mxGetM(upbound_vec);
	...
	if(upbound_vector_row_num!=prob.l)
	{
		mexPrintf("Length of upbound vector does not match # of instances.\n");
		return -1;
	}
	...
	prob.upbound = Malloc(double, prob.l);
...
int read_problem_sparse(const mxArray *label_vec, const mxArray *instance_mat, const mxArray *upbound_vec)
{
	int i, j, k, low, high;
	mwIndex *ir, *jc;
	int elements, max_index, num_samples, label_vector_row_num,upbound_vector_row_num;
	double *samples, *labels, *upbound;
...
// now assume prhs[0]: label prhs[1]: features prhs[2]:upperbound prhs[3] option
	...
	if(nrhs > 0 && nrhs < 5)
	{
		...
		if(mxIsSparse(prhs[1]))
		{
			if(param.kernel_type == PRECOMPUTED)
			{
				...
				err = read_problem_dense(prhs[0], lhs[0], prhs[2]);
